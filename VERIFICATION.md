...
## Receipt, presence and call-history update — September 27, 2026

- TypeScript, 26 application tests and 35 backend test entries pass. New tests cover delivered versus read state, exact message acknowledgements with timestamp ties, forged/oversized acknowledgements, blocked-user restrictions, session ownership, multiple devices, offline expiry, call labels/durations and preservation of complimentary lifetime Plus under store sync.
- Deployed the receipt/presence migration. Hosted audit confirms receipt RLS, Realtime publication membership, authenticated acknowledgement access and anonymous denial for acknowledgement/presence RPCs. Web production export passes.
- New clients acknowledge individual visibly rendered incoming messages as read; legacy read cursors remain a compatibility fallback. Delivery requires foreground recipient-app receipt, not push-provider acceptance. Presence uses independent device sessions with 60-second expiry and 15-second display refresh.
- Call history includes pagination, per-conversation and all-conversation views, answered duration, and correct declined/missed/cancelled labels. Duration measures time since acceptance, not media connectivity. Existing blocked/closed-conversation access rules also apply to history and presence.
- Real two-device timing, disconnected/reconnected sessions and authenticated visual interaction still require acceptance testing on the new app version. No full voice/video call or push-delivery test is claimed.
- Android 1.0.1 (versionCode 2) standalone APK builds with Java 17, passes signature verification, installs over 1.0.0 without a data reset, and launches to the live-account sign-in screen. No ReactNativeJS/AndroidRuntime errors appeared in the checked startup logs. Artifact: `.expo/builds/pigeon-post-1.0.1-preview.apk`; screenshot: `.expo/builds/apk-1.0.1-launch.png`.
