...
- Message ticks: ✓ Sent means saved by the server; ✓✓ Delivered means fetched and acknowledged by the recipient app; ✓✓ Read means visibly rendered in the active conversation. New clients acknowledge individual message IDs, so equal timestamps do not mark an unseen message read. Existing read cursors remain a fallback for older clients. Delivery is processed while the app is in the foreground; a push ticket alone never counts as delivery. Both people should install the latest APK for full status support.
- Online/offline and last-seen status appear in chat and the conversation list. Separate device/tab heartbeats expire after 60 seconds; the display refreshes every 15 seconds. Backgrounding sends an offline update. Only accepted, unblocked connections can query presence. A failed status query displays “Status unavailable.”
- Call history is available under Chats → Call history and inside each chat → Calls. It includes voice/video, incoming/outgoing, ringing, missed, declined/cancelled, dates and time since acceptance for answered calls. Duration is not a measurement of successful media connectivity. Blocking/closing a conversation also hides its history under the existing access rules.
