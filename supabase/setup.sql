...
-- BEGIN 202609260001_complimentary_plus.sql
...
-- END 202609260001_complimentary_plus.sql
insert into supabase_migrations.schema_migrations(version, name, statements) values ('202609260001', 'complimentary_plus', array['-- Reserve complimentary_lifetime for administrator-issued gifts.
-- Year 9999 is a client-compatible lifetime sentinel (PostgreSQL infinity is not a JavaScript date).
create or replace function public.apply_premium_status(p_user uuid, p_expires_at timestamptz, p_period_start timestamptz, p_product_id text, p_observed_at timestamptz default now()) returns void
language plpgsql security definer set search_path = '''' as $$
begin
...
-- BEGIN 202609260002_receipts_presence.sql
...
-- END 202609260002_receipts_presence.sql
insert into supabase_migrations.schema_migrations(version, name, statements) values ('202609260002', 'receipts_presence', array['-- Delivery is acknowledged by the recipient client, never inferred from a push ticket.
create table public.message_receipts (
 message_id uuid primary key references public.messages(id) on delete cascade,
 conversation_id uuid not null references public.conversations(id) on delete cascade,
 recipient_id uuid not null references public.profiles(id) on delete cascade,
