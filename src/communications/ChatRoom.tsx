...
import { AppState, FlatList, KeyboardAvoidingView, Platform, Pressable, Text, TextInput, View, ViewToken } from 'react-native';
...
import { C } from '../theme';
import { Attachment, Call, ChatActivity, readThrough, typing } from './api';
...
export function ChatRoom({conversationId,userId,name,premium,readingEnabled,onBack,onReport,onTranslate,onCall}:Props){
 ...
 const visibility=useRef(focused);visibility.current=focused&&readingEnabled&&AppState.currentState==='active'&&(Platform.OS!=='web'||document.visibilityState==='visible');
 ...
 acknowledge.current=()=>{const id=visibleMessage.current;if(!visibility.current||!id||lastRead.current===id)return;lastRead.current=id;void readThrough(conversationId,id).catch(()=>{if(lastRead.current===id)lastRead.current='';});};
 const onViewable=useRef(({viewableItems}:{viewableItems:ViewToken<LiveMessage>[]})=>{const last=viewableItems.filter(v=>v.isViewable).at(-1);visibleMessage.current=last?.item.id||null;acknowledge.current();}).current;
 const refresh=useCallback(async()=>{
  ...
  setActivity(a.data||[]);setFiles(f.data||[]);if(!loaded.current){setOlder(rows.length===100);loaded.current=true;}
 },[conversationId]);
 useEffect(()=>{
  ...
  for(const table of ['messages','chat_activity','chat_attachments'])channel.on('postgres_changes',{event:'*',schema:'public',table,filter:`conversation_id=eq.${conversationId}`},()=>{clearTimeout(timer);timer=setTimeout(()=>void refresh().catch(e=>{if(alive.current)setError(e.message);}),150);});
 ...
 useEffect(()=>{
  setActiveConversation(focused?conversationId:null);
  const update=()=>{visibility.current=focused&&readingEnabled&&AppState.currentState==='active'&&(Platform.OS!=='web'||document.visibilityState==='visible');if(!visibility.current)void typing(conversationId,false).catch(()=>{});else acknowledge.current();};
 ...
 },[focused,readingEnabled,conversationId]);
 ...
 return <KeyboardAvoidingView style={{flex:1,minWidth:0,backgroundColor:C.bg}} behavior={Platform.OS==='ios'?'padding':undefined}>
  ...
  <FlatList ref={list} data={messages} keyExtractor={m=>m.id} keyboardShouldPersistTaps="handled" contentContainerStyle={{padding:16,gap:12,width:'100%',maxWidth:780,alignSelf:'center'}} onScroll={e=>{const {layoutMeasurement,contentOffset,contentSize}=e.nativeEvent;atBottom.current=layoutMeasurement.height+contentOffset.y>=contentSize.height-90;}} scrollEventThrottle={100} onContentSizeChange={()=>{if(atBottom.current)list.current?.scrollToEnd({animated:false});}} onViewableItemsChanged={onViewable} viewabilityConfig={{itemVisiblePercentThreshold:60,minimumViewTime:300}}
   ...
   renderItem={({item:m})=>{const own=m.sender_id===userId;const file=files.find(f=>f.message_id===m.id);return <View style={{alignSelf:own?'flex-end':'flex-start',maxWidth:'90%',backgroundColor:own?C.green:'#E6EBDE',padding:16,borderRadius:14,gap:10}}>
    ...
    <Text style={{fontSize:11,color:own?'#E1E9DB':C.muted}}>{new Date(m.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}{own?peer&&Date.parse(peer.read_at)>=Date.parse(m.created_at)?' · Read':' · Sent':''}</Text>
   </View>;}}/>
